...
func main() {
	...
	ourShader := shader.MakeShaders("1.2.pbr.vs", "1.2.pbr.fs")
	ourShader.Use()
	...
	dir := "../../../resources/textures/pbr/rusted_iron"
	albedo := loadModel.TextureFromFile("albedo.png", dir, false)
	normal := loadModel.TextureFromFile("normal.png", dir, false)
	metallic := loadModel.TextureFromFile("metallic.png", dir, false)
	roughness := loadModel.TextureFromFile("roughness.png", dir, false)
	ao := loadModel.TextureFromFile("ao.png", dir, false)

	lightPositions := []mgl32.Vec3{
		mgl32.Vec3{0.0, 0.0, 10.0},
	}
	lightColors := []mgl32.Vec3{
		mgl32.Vec3{150.0, 150.0, 150.0},
	}
	nrRows := 7
	nrCols := 7
	spacing := float32(2.5)
